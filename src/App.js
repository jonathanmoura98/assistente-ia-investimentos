...
const App = () => {
  ...
  const [initialSavings, setInitialSavings] = useState(0); // Novo estado: Capital inicial
  const [investmentType, setInvestmentType] = useState('CDI'); // Tipo de investimento padrão
  const [customAnnualReturn, setCustomAnnualReturn] = useState(0.07); // Novo estado: Taxa de retorno anual personalizada (padrão 7%)
  const [inflationRate, setInflationRate] = useState(0.04); // Novo estado: Taxa de inflação anual (padrão 4%)
  ...
  const handleAskAI = async (queryToAsk = userQuery) => { // Aceita uma query opcional
    setError('');
    setAiResponse('');
    setIsLoading(true);
    ...
    try {
      let chatHistory = [];
      const prompt = `Você é um assistente de IA focado em educar pequenos investidores sobre conceitos financeiros e de investimento. Forneça explicações claras, concisas e fáceis de entender. Evite jargões complexos e concentre-se em fundamentos. Responda à seguinte pergunta: "${queryToAsk}"`;
      ...
      if (result.candidates && result.candidates.length > 0 &&
        ...
        setAiResponse('Não foi possível obter uma resposta. Tente novamente.');
      }
    } catch (err) {
  ...
  const handleGenerateStrategy = async () => {
    ...
    try {
      let chatHistory = [];
      const prompt = `Você é um consultor financeiro de IA para pequenos investidores. Com base na tolerância ao risco de "${riskTolerance}" e no objetivo de investimento de "${investmentGoal}", forneça uma estratégia de investimento geral e clara. Explique os princípios-chave e dê exemplos de tipos de ativos adequados. Mantenha a linguagem simples e focada em educação.`;
      chatHistory.push({ role: "user", parts: [{ text: prompt }] });
      ...
      if (result.candidates && result.candidates.length > 0 &&
          ...
          result.candidates[0].content.parts.length > 0) {
        const text = result.candidates[0].content.parts[0].text;
        setInvestmentStrategy(text);
        speakText(text); // Falar a estratégia
      } else {
        setInvestmentStrategy('Não foi possível gerar uma estratégia. Tente novamente.');
      }
    } catch (err) {
  ...
  const calculateFinancialIndependence = () => {
    ...
    if (months >= 1200) {
      resultText = `Com um investimento mensal de R$ ${monthlyInvestmentAmount.toLocaleString('pt-BR')} e capital inicial de R$ ${initialSavings.toLocaleString('pt-BR')} em ${investmentType} (retorno de ${(annualRate * 100).toFixed(1)}% a.a., inflação de ${(inflationRate * 100).toFixed(1)}% a.a.), pode levar mais de 100 anos para atingir a independência financeira com despesas anuais alvo de R$ ${targetAnnualExpenses.toLocaleString('pt-BR')}. Considere aumentar o investimento ou reduzir as despesas.`;
    } else {
      resultText = `Com um investimento mensal de R$ ${monthlyInvestmentAmount.toLocaleString('pt-BR')} e capital inicial de R$ ${initialSavings.toLocaleString('pt-BR')} em ${investmentType} (retorno de ${(annualRate * 100).toFixed(1)}% a.a., inflação de ${(inflationRate * 100).toFixed(1)}% a.a.), levaria aproximadamente ${years} anos para atingir a independência financeira, considerando despesas anuais alvo de R$ ${targetAnnualExpenses.toLocaleString('pt-BR')}.`;
    }
